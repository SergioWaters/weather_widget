<template>
  <div class="weather__page">
    <button class="options_btn" @click="showOptions">
      <svg
    ...
    <div class="placeholder" v-if="!getSavedLocsOrder.length">
      No saved locations. You may add one with settings.
    </div>
    <div class="weather__list">
      <WeatherCard
        v-for="loc of getSavedLocsOrder"
        :weather="getSavedLocs[loc]"
        :key="loc"
      />
    </div>
    <h4 v-if="getError">{{ getError }}</h4>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapGetters(["getSavedLocs", "getSavedLocsOrder", "getError"]),
  },
  emits: ["showOptions"],
  methods: {
    ...
    fetchWeather() {
      ...
      this.getSavedLocsOrder.forEach((loc) => {
        this.getWeather(this.getSavedLocs[loc]);
      });
    },
    showOptions() {
  ...
  mounted() {
    if (!this.getSavedLocsOrder.length) this.isOptionsShown = true;
    this.interval = setInterval(this.fetchWeather, 60000);
  },
};
</script>
